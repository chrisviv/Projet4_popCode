
// // on initialise une variable let i
// let i =28;

// //Tant que
// while (i<28){
//     //...exécute ce code
//     document.getElementById('nomLangage').innerHTML += 'i la valeur ' + '/ 28' + (i + 1);
//     x++;
// }
// //sortie de la boucle quand arrive à 28 'condition de sortie'

// for (let i =0; i<28; i++){
//     document.getElementById('nomLangage'.innerHTML +=
//     'i stock la valeur' + i + 'lors du passage N°' + (i + 1) + 'dans la boucle<br>';}



// ***************************************************//
















// let nomLangage = document.querySelector('.nomLangage');
// let nomLangages ='';
// JUSTE POUR AFFICHER PAS RESULTAT DEMANDER
// let i
//         for (i = 0; i < nomLanage.length; i++) {
//             nomLangages += "<li>" + listLangages[i].toUpperCase() + "</li>";
//         }
  
//         document.querySelector(".nomLangage").innerHTML = nomLangages;